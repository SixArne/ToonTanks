...
ABasePawn::ABasePawn()
{
	...
	m_BaseMesh = CreateDefaultSubobject<UStaticMeshComponent>(TEXT("Mesh"));
	m_BaseMesh->SetupAttachment(m_Collider);

	m_TurretMesh = CreateDefaultSubobject<UStaticMeshComponent>(TEXT("TurretMesh"));
	m_TurretMesh->SetupAttachment(m_BaseMesh);

	m_ProjectileSpawnPoint = CreateDefaultSubobject<USceneComponent>(TEXT("ProjectileSpawnPoint"));
